...
} from "./variableTypeWithDefaultValue";
import { ObjectSetLocators } from "./variableValues";
...
export interface ExecutableEvent {
   ...
   *
   * Returns false if the event was not able to be executed (e.g. the CTRL button
   * was held down, but there was no way to execute the transition in a new tab,
   * and so it was deemed safer to not execute it at all, instead of risk overwriting
...
export type ValueAndSetterMethods<T extends IVariableType_WithDefaultValue> = {
   ...
   */
  fieldValue?: IAsyncLoaded<VariableTypeToValueType<T>>;
...
/**
 * Mapepd type to extract field types on structs.
 */
